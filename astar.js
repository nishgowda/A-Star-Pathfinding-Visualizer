...
$(document).ready(function () {
    ...
    function addNeighbors(currentNode) {
        ...
        for (let i = 0; i < neighbors.length; i++) {
            var n = {
                x: currentNode.x + neighbors[i].x,
                y: currentNode.y + neighbors[i].y,
                g: 0,
                h: 0,
                prt: {
                    x: currentNode.x,
            ...
            // if current node x and y dist are not the same or if the node is not a neighbor or if node is a wall
    ...
    $("#startNode").click(function () {
        ...
        function mouseFill(e) {
            ...
            if (e.which != 1 && ! touchstart) 
                return;
            
            pixel = [
                Math.floor(offsetX / pixelSizeX),
                Math.floor(offsetY / pixelSizeY)
            ];
            fillPixel(pixel);
            startENABLED = false;
        }
    ...
    // Using JQuery to allow the user to add the end
    // node to board.
    endENABLED = true;
    ...
    $("#walls").click(function () {
        ...
        function fillPixel(pixel) {
            ...
            ctx.fillRect(pixel[0] * pixelSizeX, pixel[1] * pixelSizeY, pixelSizeX - 1, pixelSizeY - 1);
            walls.push({x: pixel[0], y: pixel[1]});
        }
        // Allows user to delete any of the walls they created if they hold Shift and click on the wall.
        // If the wall is deleted from the map, the wall is popped from the array given the pixel coordinate.
        canvas.on('mousemove touchmove touchstart mousedown', mouseUnfill);
        function mouseUnfill(e) {
            e.preventDefault();
            let offsetX = e.offsetX;
            let offsetY = e.offsetY;
            if (! e.shiftKey && e.keyCode != 1) 
                return;
    ...
    // Highlights select box when mouse hovers over a pixel. Using JQuery to create the
    // selector for the mouse cursor and prepend it to
    // the wrapper div in HTML
    function selectBox() {
        canvas.mousemove(function (e) {
            let pixel = [
                Math.floor(e.offsetX / (pixelSizeX)),
                Math.floor(e.offsetY / (pixelSizeY))
            ];
            if (!SELECTEDBOX) {
    ...
    function createMap() {
        ...
        map[5][3] = map[6][3] = map[7][3] = map[3][4] = map[7][4] = map[3][5] = map[7][5] = map[3][6] = map[4][6] = map[5][6] = map[6][6] = map[7][6] = 1;
    }

    // The run function, given a click even with JQuery, when the user
    // adds a start and end node (also walls if they wish), then run the algorithm.
    $("#run").click(function () {
        startNode = {
            x: startNodeX,
            y: startNodeY,
